...
# MAGIC %md
# MAGIC # LiDAR U-Next — Experiments & Debugging
# MAGIC
# MAGIC Scratch notebook for iterative experimentation. Test config changes, debug errors,
# MAGIC and try new ideas before promoting to `04_unext_multigpu`.
# MAGIC
...
spark.sql("CREATE VOLUME IF NOT EXISTS chlor.lidar_schema.tmp")
CHUNK_DIR = "/Volumes/chlor/lidar_schema/tmp/lidar_chunks"
...
import os as _os
_os.makedirs("/Volumes/chlor/lidar_schema/tmp", exist_ok=True)
with open("/Volumes/chlor/lidar_schema/tmp/unext_model.py", "w") as f:
    f.write(UNEXT_CODE)
print("U-Next model written to /Volumes/chlor/lidar_schema/tmp/unext_model.py")
...
def train_ddp():
    ...
    chunk_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp/lidar_chunks/chunk_*.pkl"))
    ...
    if rank == 0:
        ...
        torch.save(raw_model.state_dict(), "/Volumes/chlor/lidar_schema/tmp/unext_final.pt")
        import mlflow
...
def _train_ddp_safe():
    ...
    try:
        ...
        err_path = f"/Volumes/chlor/lidar_schema/tmp/error_rank{rank}.txt"
...
# Pass Databricks credentials to worker processes
...
print(f"Launching U-Next training ({N_WORKERS} GPUs, local_mode=True)...")
TorchDistributor(num_processes=N_WORKERS, local_mode=True, use_gpu=True).run(_train_ddp_safe)
print("Training finished.")

# Dump any worker error files
import glob as _glob
for _ef in sorted(_glob.glob("/Volumes/chlor/lidar_schema/tmp/error_rank*.txt")):
    print(f"\n=== {_ef} ===")
    print(open(_ef).read())
...
import torch
import sys
sys.path.insert(0, "/Volumes/chlor/lidar_schema/tmp")
...
model.load_state_dict(torch.load("/Volumes/chlor/lidar_schema/tmp/unext_final.pt", map_location=device))
...
all_true, all_pred = [], []
sample_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp/lidar_chunks/chunk_*.pkl"))[:50]  # evaluate on 50 chunks
...
# MAGIC | Distribution | TorchDistributor / PyTorch DDP (single GPU, driver) |
