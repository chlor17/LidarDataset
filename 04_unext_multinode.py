...
# MAGIC %md
# MAGIC # LiDAR Semantic Segmentation — U-Next (Multi-Node, 4 GPU Workers)
# MAGIC
...
LR          = 1e-3
N_WORKERS   = 4       # number of GPU workers for TorchDistributor
...
spark.sql("CREATE VOLUME IF NOT EXISTS chlor.lidar_schema.tmp_multinode")
CHUNK_DIR = "/Volumes/chlor/lidar_schema/tmp_multinode/lidar_chunks"
...
import os as _os
_os.makedirs("/Volumes/chlor/lidar_schema/tmp_multinode", exist_ok=True)
with open("/Volumes/chlor/lidar_schema/tmp_multinode/unext_model.py", "w") as f:
    f.write(UNEXT_CODE)
print("U-Next model written to /Volumes/chlor/lidar_schema/tmp_multinode/unext_model.py")
...
def train_ddp():
    ...
    chunk_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp_multinode/lidar_chunks/chunk_*.pkl"))
    ...
    if rank == 0:
        ...
        torch.save(raw_model.state_dict(), "/Volumes/chlor/lidar_schema/tmp_multinode/unext_final.pt")
        import mlflow
...
def _train_ddp_safe():
    ...
    try:
        ...
        err_path = f"/Volumes/chlor/lidar_schema/tmp_multinode/error_rank{rank}.txt"
...
# Capture credentials + hyperparams in closure — cloudpickle serializes free vars to remote workers
# (os.environ set on driver is NOT propagated to local_mode=False workers)
_ctx = dbutils.notebook.entry_point.getDbutils().notebook().getContext()
_DB_HOST  = _ctx.apiUrl().getOrElse(None)
_DB_TOKEN = _ctx.apiToken().getOrElse(None)

# Hyperparams as closure vars (mirrors the os.environ block above)
_n_points    = N_POINTS
...
def _train_with_creds():
    import os
    os.environ["DATABRICKS_HOST"]  = _DB_HOST
    os.environ["DATABRICKS_TOKEN"] = _DB_TOKEN
    # NCCL tuning for multi-node — must be set in closure (driver env doesn't reach workers)
    os.environ["NCCL_P2P_DISABLE"]    = "1"
    os.environ["NCCL_SOCKET_IFNAME"]  = "eth0"
    os.environ["NCCL_IB_DISABLE"]     = "1"
    # Pass hyperparams (driver os.environ not propagated to remote workers)
    os.environ["UNEXT_N_POINTS"]    = str(_n_points)
    os.environ["UNEXT_K_NEIGHBORS"] = str(_k_neighbors)
    os.environ["UNEXT_N_FEATURES"]  = str(_n_features)
    os.environ["UNEXT_NUM_CLASSES"] = str(_num_classes)
    os.environ["UNEXT_N_EPOCHS"]    = str(_n_epochs)
    os.environ["UNEXT_BATCH_SIZE"]  = str(_batch_size)
    os.environ["UNEXT_LR"]          = str(_lr)
    os.environ["UNEXT_MODEL_NAME"]  = _model_name
    os.environ["UNEXT_EXPERIMENT"]  = _experiment
    _train_ddp_safe()
...
print(f"Launching U-Next training ({N_WORKERS} GPUs, local_mode=False)...")
TorchDistributor(num_processes=N_WORKERS, local_mode=False, use_gpu=True).run(_train_with_creds)
print("Training finished.")

# Dump any worker error files
import glob as _glob
for _ef in sorted(_glob.glob("/Volumes/chlor/lidar_schema/tmp_multinode/error_rank*.txt")):
    print(f"\n=== {_ef} ===")
    print(open(_ef).read())
...
import torch
import sys
sys.path.insert(0, "/Volumes/chlor/lidar_schema/tmp_multinode")
...
model.load_state_dict(torch.load("/Volumes/chlor/lidar_schema/tmp_multinode/unext_final.pt", map_location=device))
...
all_true, all_pred = [], []
sample_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp_multinode/lidar_chunks/chunk_*.pkl"))[:50]  # evaluate on 50 chunks
...
# MAGIC | Distribution | TorchDistributor / PyTorch DDP (4 workers, local_mode=False) |
