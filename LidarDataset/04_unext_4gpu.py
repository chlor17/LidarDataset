...
# MAGIC %md
# MAGIC # LiDAR Semantic Segmentation — U-Next (Single Node, 4 GPU)
# MAGIC
...
spark.sql("CREATE VOLUME IF NOT EXISTS chlor.lidar_schema.tmp_4gpu")
CHUNK_DIR = "/Volumes/chlor/lidar_schema/tmp_4gpu/lidar_chunks"
...
import os as _os
_os.makedirs("/Volumes/chlor/lidar_schema/tmp_4gpu", exist_ok=True)
with open("/Volumes/chlor/lidar_schema/tmp_4gpu/unext_model.py", "w") as f:
    f.write(UNEXT_CODE)
print("U-Next model written to /Volumes/chlor/lidar_schema/tmp_4gpu/unext_model.py")
...
def train_ddp():
    ...
    chunk_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp_4gpu/lidar_chunks/chunk_*.pkl"))
    ...
    if rank == 0:
        ...
        torch.save(raw_model.state_dict(), "/Volumes/chlor/lidar_schema/tmp_4gpu/unext_final.pt")
        import mlflow
...
def _train_ddp_safe():
    ...
    try:
        ...
        err_path = f"/Volumes/chlor/lidar_schema/tmp_4gpu/error_rank{rank}.txt"
...
import glob as _glob
for _ef in sorted(_glob.glob("/Volumes/chlor/lidar_schema/tmp_4gpu/error_rank*.txt")):
    print(f"\n=== {_ef} ===")
    print(open(_ef).read())
...
import torch
import sys
sys.path.insert(0, "/Volumes/chlor/lidar_schema/tmp_4gpu")
...
model.load_state_dict(torch.load("/Volumes/chlor/lidar_schema/tmp_4gpu/unext_final.pt", map_location=device))
...
all_true, all_pred = [], []
sample_files = sorted(glob.glob("/Volumes/chlor/lidar_schema/tmp_4gpu/lidar_chunks/chunk_*.pkl"))[:50]  # evaluate on 50 chunks
...
# MAGIC | Distribution | TorchDistributor / PyTorch DDP (4 GPU, local_mode=True) |
